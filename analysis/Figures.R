...
enrol.fig <- ggplot(ecec.plot.df, aes(x = year, y = enrolment)) +
  ...
  ggtitle(label = "Percentage of 0-to-2-year-olds enrolled in formal childcare and preschool institutions (EU28, 2004-2013)") +
  theme_minimal() +
...
enrol.fig <- enrol.fig + geom_text_repel(data = subset(mean.enrolment, year == 2005), 
